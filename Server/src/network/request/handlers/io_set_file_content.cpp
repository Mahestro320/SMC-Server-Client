#include "network/request/handlers/io_set_file_content.hpp"

#include <cstdlib>
#include <filesystem>
#include <string>
#include <system_error>
#include <vector>

#include "Shared/io/console.hpp"
#include "Shared/network/response/error.hpp"
#include "Shared/pds.hpp"
#include "Shared/util/error_message.hpp"
#include "network.hpp"
#include "network/request/handler.hpp"
#include "server/config.hpp"

namespace fs = std::filesystem;

IOSetFileContentRH::IOSetFileContentRH() : RH{REQUIREMENTS} {
}

IOSetFileContentRH::~IOSetFileContentRH() {
    closeFile();
}

void IOSetFileContentRH::_run() {
    if (!extractPath() || !extractFileSize() || !extractBufferSize() || !extractBufferCount() || !checkBufferSize() ||
        !openFile()) {
        return;
    }
    if (!sendResponse()) {
        closeFile();
        return;
    }
    transmitData();
    closeFile();
}

bool IOSetFileContentRH::extractPath() {
    return request.extractAbsPath(*client, target_path, response);
}

bool IOSetFileContentRH::extractFileSize() {
    return request.extractInt<pds::file_size_t>(file_size);
}

bool IOSetFileContentRH::extractBufferCount() {
    return request.extractInt<pds::network_buffer_count_t>(buffer_count);
}

bool IOSetFileContentRH::extractBufferSize() {
    return request.extractInt<pds::network_buffer_t>(buffer_size);
}

bool IOSetFileContentRH::checkBufferSize() {
    const Config& config{Config::getInstance()};
    const ConfigValues& config_values{config.getValues()};
    if (buffer_size == 0 || buffer_size > config_values.io_load_max_buffer_size) {
        response.buildError(ResponseErrorInfos{
            .error_id = ResponseError::IOInvalidBufferSize,
            .precision_msg = "invalid buffer size: " + std::to_string(buffer_size) + " (minimum is 1, maximum is " +
                             std::to_string(config_values.io_load_max_buffer_size) + ")",
        });
        return false;
    }
    return true;
}

bool IOSetFileContentRH::openFile() {
    const fs::path dir_to_create{target_path.parent_path()};
    std::error_code ec{};
    fs::create_directories(dir_to_create, ec);
    (void)ec;

    file.open(target_path, std::ios::binary | std::ios::beg);
    if (!file.is_open() || !file.good()) {
        response.buildError(ResponseErrorInfos{
            .error_id = ResponseError::IOOpenFileError,
            .precision_msg = "cannot open file \"" + target_path.string() + "\": " + util::error_message::get(errno),
        });
        return false;
    }
    return true;
}

bool IOSetFileContentRH::transmitData() {
    console::out::inf("sending " + std::to_string(buffer_count) + " buffers");
    std::vector<char> buffer(buffer_size);
    for (pds::network_buffer_count_t i{}; i < buffer_count; ++i) {
        if (i == buffer_count - 1 && file_size % buffer_size != 0) {
            buffer.resize(static_cast<size_t>(file_size % buffer_size));
        }
        if (!network::readBuffer(client->getSocket(), buffer, true)) {
            return false;
        }
        file.write(buffer.data(), static_cast<std::streamsize>(buffer.size()));
    }
    return true;
}

void IOSetFileContentRH::closeFile() {
    if (file.is_open()) {
        file.close();
    }
}
